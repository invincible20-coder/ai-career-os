    ...
    <div className="flex items-center gap-2 overflow-x-auto px-1 py-1">
      {presets.map((preset, i) => (
        <motion.button
          ...
          className={cn(
            "group flex items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-left transition-all hover:border-teal-500/20 hover:bg-teal-500/5 disabled:opacity-50",
            "shrink-0"
          )}
        >
          <div className="flex h-7 w-7 items-center justify-center rounded-md bg-white/[0.04] group-hover:bg-teal-500/10 transition-colors">
            <Zap className="h-3.5 w-3.5 text-slate-500 group-hover:text-teal-400 transition-colors" />
          </div>
          <div>
